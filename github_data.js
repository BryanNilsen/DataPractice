...
function totalCommits() {
  ...
  for (let i = 0; i < githubData.length; i++) {
      if (githubData[i].payload.hasOwnProperty("commits")) {        
          commitCounter += githubData[i].payload.commits.length;
      } else if (githubData[i].type === "PullRequestEvent") {
      commitCounter += githubData[i].payload.pull_request.commits;
      }
  }
}
totalCommits()
console.log("Challenge 1 - Commits Counter: ", commitCounter)
...
function totalEventTypes() {
  for (let i = 0; i < githubData.length; i++) {
      let eventType = githubData[i].type;
      eventTypesArray.push(eventType);
  };
}
totalEventTypes();

// loops through array and creates object with event type and count
let counts = {};
for (var i = 0; i < eventTypesArray.length; i++) {
  counts[eventTypesArray[i]] = 1 + (counts[eventTypesArray[i]] || 0);
}

console.log("Challenge 2 - Type of Event with Count: ", counts)
...
function userPullRequests() {
  // cycle all github data
  for( let i = 0; i < githubData.length; i++) {
    if (githubData[i].payload.hasOwnProperty("pull_request")
        && githubData[i].payload.pull_request.hasOwnProperty("user")
        && githubData[i].payload.pull_request.user.hasOwnProperty("login")) {
      let userId = githubData[i].payload.pull_request.user;
      usersPRs.push(userId.login)
    // } else {
    }
  }
}
userPullRequests()
console.log("Challenge 3 - Github Users who submitted PRs", usersPRs)
...
listRepos()
console.log("Challenge 4 - The Repos are: ", steveRepos)
